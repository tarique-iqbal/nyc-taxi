...
- `producer.py` — reads Parquet, runs domain pipeline, publishes to Kafka; `lifecycle.startup()` also starts a Prometheus `/metrics` server on `9100 (from .env)`
- `consumer.py` — consumes Kafka, inserts into ClickHouse, commits offset after insert; `lifecycle.startup()` also starts a Prometheus `/metrics` server on `9101 (from .env)`, and a `LagMonitorLoop` polls consumer group lag on a background thread
